...
async function checkFav(check_note: Misskey.entities.Note): Promise<boolean> {
...
checkFav(appearNote).then((result) => {
...
function onContextmenu(ev: PointerEvent): void {
	...
	if (prefer.s.useReactionPickerForContextMenu) {
		...
		const { menu, cleanup } = getNoteMenu({ note: note, translating, translation, denyaize });
...
async function toggleFavorite(favorite: boolean) {
	...
	const result = await checkFav(appearNote);
	favorited.value = result;
}
